...
class Simulation:
    ...
    def run_tasks(self, num_tasks):
        ...
        while total_generated_tasks <= num_tasks:
            if total_generated_tasks < num_tasks:
                count_generated_task, generated_tasks = self.task_generator.generate_tasks(None, task_generation_prob=0.3, task_size_min=10, task_size_max=1000)
                total_generated_tasks += len(generated_tasks)
                Scheduler(simulation_instance=self).apply(generated_tasks, "ASCO")
                self.global_clock += 1
                self.generated_tasks += count_generated_task
                for task in generated_tasks:
                    if task.execution_server:
                        self.completed_tasks += 1
                        self.total_cost += self.server_Selector.calculate_total_cost(task, task.execution_server)
    ...
    def visualize_results(self):
        labels = ['Generated Tasks', 'Completed Tasks']
        values = [self.generated_tasks, self.completed_tasks]

        plt.figure(figsize=(10, 6))
        plt.subplot(1, 2, 1)
        plt.bar(labels, values, color=['blue', 'green'])
        plt.title('Task Generation and Completion')
        plt.ylabel('Number of Tasks')

        average_cost_per_task = self.total_cost / self.completed_tasks if self.completed_tasks > 0 else 0

        plt.subplot(1, 2, 2)
        plt.bar(['Average Cost per Task'], [average_cost_per_task], color='red')
        plt.title('Average Cost per Task')
        plt.ylabel('Cost')
...
def main():
    num_tasks = 100
    time = 100
    network = NetworkGenerator(num_clusters=20, num_devices=15)
    ...
    proposed_simulation.visualize_results()
